...
public class DemoNoSQLTableUserProfile extends DemoNoSQLTableBase {
    private static final String LOG_TAG = DemoNoSQLTableUserProfile.class.getSimpleName();
    ...
    public class DemoGetWithPartitionKeyAndSortKey extends DemoNoSQLOperationBase {
        ...
        DemoGetWithPartitionKeyAndSortKey(final Context context) {
            super(context.getString(R.string.nosql_operation_get_by_partition_and_sort_text),
                String.format(context.getString(R.string.nosql_operation_example_get_by_partition_and_sort_text),
                    ...
                    "phone", "demo-userId-500000"));
        }

        @Override
        public boolean executeOperation() {
            // Retrieve an item by passing the partition key using the object mapper.
            result = mapper.load(UserProfileDO.class, AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID(), "demo-phone-500000");
        ...
        public List<DemoNoSQLResult> getNextResultGroup() {
            ...
            results.add(new DemoNoSQLUserProfileResult(result));
    ...
    public class DemoQueryWithPartitionKeyAndSortKeyCondition extends DemoNoSQLOperationBase {
        ...
        private PaginatedQueryList<UserProfileDO> results;
        private Iterator<UserProfileDO> resultsIterator;
        ...
        @Override
        public boolean executeOperation() {
            final UserProfileDO itemToFind = new UserProfileDO();
            itemToFind.setUserId(AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID());

            final Condition rangeKeyCondition = new Condition()
                .withComparisonOperator(ComparisonOperator.LT.toString())
                .withAttributeValueList(new AttributeValue().withS("demo-phone-500000"));
            final DynamoDBQueryExpression<UserProfileDO> queryExpression = new DynamoDBQueryExpression<UserProfileDO>()
                .withHashKeyValues(itemToFind)
                .withRangeKeyCondition("phone", rangeKeyCondition)
                .withConsistentRead(false)
                .withLimit(RESULTS_PER_RESULT_GROUP);

            results = mapper.query(UserProfileDO.class, queryExpression);
            if (results != null) {
    ...
    public class DemoScanWithFilter extends DemoNoSQLOperationBase {
        ...
        private PaginatedScanList<UserProfileDO> results;
        private Iterator<UserProfileDO> resultsIterator;
        ...
        public boolean executeOperation() {
            ...
            results = mapper.scan(UserProfileDO.class, scanExpression);
            if (results != null) {
                resultsIterator = results.iterator();
                if (resultsIterator.hasNext()) {
                    return true;
     ...
     */
    private static List<DemoNoSQLResult> getNextResultsGroupFromIterator(final Iterator<UserProfileDO> resultsIterator) {
        if (!resultsIterator.hasNext()) {
            return null;
        }
        ...
        do {
            ...
            final UserProfileDO item = resultsIterator.next();
            // Add the item to a group of results that will be displayed later.
            resultGroup.add(new DemoNoSQLUserProfileResult(item));
            itemsRetrieved++;
    ...
    public DemoNoSQLTableUserProfile() {
    ...
    public void insertSampleData() throws AmazonClientException {
        ...
        final UserProfileDO firstItem = new UserProfileDO();

        firstItem.setUserId(AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID());
        firstItem.setPhone("demo-phone-500000");
        firstItem.setName(
            DemoSampleDataGenerator.getRandomSampleString("name"));
        firstItem.setPushTargetArn(DemoSampleDataGenerator.getSampleStringSet());
        ...
        final UserProfileDO[] items = new UserProfileDO[SAMPLE_DATA_ENTRIES_PER_INSERT-1];
        for (int count = 0; count < SAMPLE_DATA_ENTRIES_PER_INSERT-1; count++) {
            final UserProfileDO item = new UserProfileDO();
            item.setUserId(AWSMobileClient.defaultMobileClient().getIdentityManager().getCachedUserID());
            item.setPhone(DemoSampleDataGenerator.getRandomSampleString("phone"));
            item.setName(DemoSampleDataGenerator.getRandomSampleString("name"));
            item.setPushTargetArn(DemoSampleDataGenerator.getSampleStringSet());
    ...
    public void removeSampleData() throws AmazonClientException {
        ...
        final PaginatedQueryList<UserProfileDO> results = mapper.query(UserProfileDO.class, queryExpression);

        Iterator<UserProfileDO> resultsIterator = results.iterator();
        ...
        final List<UserProfileDO> batchOfItems = new LinkedList<UserProfileDO>();
